...
def get_today_stocks_rates(mt5_conn, stocks_dict):
    """Extracts today stocks rates from a stock list
    ...
    rates_df = pd.DataFrame(columns=['DATE_TIME', 'OPEN', 'HIGH', 'LOW', 'CLOSE', 
        'PROFIT_FACTOR', 'CANDLE_HALF', 'DIFF_FROM_CLOSE_TO_HALF', 'DIFF_FROM_CLOSE_TO_LOW'])
    ...
    for stock in stocks_dict.keys():
        try: 
            ...
            rates_df.loc[stock] = [date_time, open, high, low, close, 
                profit_factor, candle_half, diff_from_close_to_half, diff_from_close_to_low]
        except:
            continue
    return rates_df
...
def get_good_stocks_rates(mt5_conn, stocks_dict):
    ...
    good_stocks_df = pd.DataFrame(columns=['DATE_TIME', 'OPEN', 'HIGH', 'LOW', 'CLOSE', 
        'PROFIT_FACTOR', 'CANDLE_HALF', 'DIFF_FROM_CLOSE_TO_HALF', 'DIFF_FROM_CLOSE_TO_LOW'])
